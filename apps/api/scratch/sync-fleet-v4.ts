...
async function syncFleetV4() {
  ...
  try {
    console.log('🔱 ARCHON FLEET SYNCHRONIZER V4: TOTAL PARITY');
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const data = {
        ...
        // V4 EXTENSIONS
        color: normalizeString(cols[27]),
        capacidadCarga: parseFloat(cols[28]) || 0,
        fuelTankCapacity: parseFloat(cols[29]) || 0
      };

      console.log(`Syncing ${id}: [Color: ${data.color}] [Load: ${data.capacidadCarga}]`);
      ...
      await connection.execute(`
        ...
        SET 
          ...
          description = ?, -- Mapping color to description for display
          capacidadCarga = ?,
          fuelTankCapacity = ?
        WHERE id = ?
      `, [
    ...
    console.log('🔱 SYNCHRONIZATION V4 COMPLETE: IMPECCABILITY ACHIEVED');
