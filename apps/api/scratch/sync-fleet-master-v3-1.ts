...
function normalizeString(str: string): string {
  if (!str) return '';
  // 🛡️ Replace common encoding artifacts
  return str
    .replace(/\uFFFD/g, 'e') // Replacement character
...
async function syncFleetEncrypted() {
  ...
  try {
    console.log('🔱 ARCHON FLEET SYNCHRONIZER V3.1: UTF-8 HARDENING');
    ...
    // Read as binary first to detect encoding if necessary, but we'll assume UTF-8 with potential artifacts
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      console.log(`Syncing ${id}: [UTF-8 Path Activated]`);
    ...
    console.log('🔱 SYNCHRONIZATION V3.1 COMPLETE: ZERO NOISE');
