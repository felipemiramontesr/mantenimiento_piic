...
async function run() {
  // STEP 1: Expand column sizes to accommodate encrypted data
  console.log('Step 1: Expanding column sizes...');
  await db.execute('ALTER TABLE fleet_units MODIFY COLUMN placas VARCHAR(255) DEFAULT NULL');
  await db.execute('ALTER TABLE fleet_units MODIFY COLUMN numeroSerie VARCHAR(255) DEFAULT NULL');
  await db.execute('ALTER TABLE fleet_units MODIFY COLUMN circulationCardNumber VARCHAR(255) DEFAULT NULL');
  console.log('✅ Columns expanded to VARCHAR(255)');

  // STEP 2: Re-encrypt and store
  console.log('\nStep 2: Re-encrypting all data...');
  for (const [id, data] of Object.entries(unitData)) {
    ...
    const encTc = EncryptionService.encrypt(tarjetas[id]);
  ...
  console.log('✅ All 23 units re-encrypted');

  // STEP 3: Verify roundtrip
  console.log('\nStep 3: Verifying decrypt roundtrip...');
  const [rows] = await db.execute('SELECT id, placas, numeroSerie, circulationCardNumber FROM fleet_units WHERE is_active = 1 ORDER BY id');
  let allGood = true;
  for (const row of rows as any[]) {
    ...
    const expected = unitData[row.id];
    const match = p === expected?.placas && s === expected?.serie;
    if (!match) allGood = false;
    console.log(`${match ? '✅' : '❌'} ${row.id} -> Placas: ${p} | VIN: ${s} | TC: ${tc}`);
  }

  if (allGood) {
    console.log('\n🎯 100% DECRYPT PARITY — All fields decrypt correctly.');
  } else {
    console.log('\n❌ SOME FIELDS FAILED DECRYPTION');
  }
