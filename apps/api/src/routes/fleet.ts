...
const createFleetSchema = z.object({
  ...
  transmision:             z.enum(['Automática', 'Estándar (Manual)', 'CVT', 'Hidrostática', 'N/A']).default('N/A'),
...
export default async function fleetRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post('/fleet', async (request, reply) => {
    ...
    const {
      assetType, tag, numeroSerie, marca, modelo, year, motor,
      traccion, transmision, fuelType, tireSpec, tireBrand,
      capacidadCarga, odometer, sede, centroMantenimiento,
      vigenciaSeguro, vencimientoVerificacion, tarjetaCirculacion,
      status, assignedOperatorId,
    } = parse.data;
    ...
    try {
      ...
      const [existing] = await db.execute<FleetUnit[]>(
        'SELECT id FROM fleet_units WHERE tag = ?', [tag]
      );
      if (existing.length > 0) {
        return reply.code(409).send({ error: `Número Económico '${tag}' ya existe en el registro` });
      }

      // Check for duplicate numero_serie
      if (numeroSerie) {
        const [existingSerie] = await db.execute<FleetUnit[]>(
          'SELECT id FROM fleet_units WHERE numero_serie = ?', [numeroSerie]
        );
        if (existingSerie.length > 0) {
          return reply.code(409).send({ error: `Número de serie '${numeroSerie}' ya existe en el registro` });
        }
      }
      ...
      await db.execute(
        ...
        [
          nextId, uuid, assetType, tag, numeroSerie || null, marca, modelo, year,
          motor || null, traccion, transmision, fuelType,
          tireSpec || null, tireBrand || null, capacidadCarga || null,
          odometer, sede || null, centroMantenimiento,
          vigenciaSeguro || null, vencimientoVerificacion || null,
          tarjetaCirculacion || null, status, assignedOperatorId || null,
        ]
      );
  ...
  fastify.patch('/fleet/:id', async (request, reply) => {
    ...
    if (raw.centroMantenimiento     !== undefined) updates.centro_mantenimiento    = raw.centroMantenimiento;
    if (raw.vigenciaSeguro          !== undefined) updates.vigencia_seguro         = raw.vigenciaSeguro ?? null;
    if (raw.vencimientoVerificacion !== undefined) updates.vencimiento_verificacion = raw.vencimientoVerificacion ?? null;
    if (raw.tarjetaCirculacion      !== undefined) updates.tarjeta_circulacion     = raw.tarjetaCirculacion ?? null;
    if (raw.status                  !== undefined) updates.status                  = raw.status;
    if (raw.assignedOperatorId      !== undefined) updates.assigned_operator_id   = raw.assignedOperatorId ?? null;
    ...
    try {
      ...
      const [result] = await db.execute(
        `UPDATE fleet_units SET ${setClause} WHERE id = ?`, values
      );
