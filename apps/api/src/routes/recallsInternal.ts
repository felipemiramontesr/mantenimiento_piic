...
/** Shapes one `view_fleet_model_failure_patterns` row into the API response row. */
function mapPatternRow(p: RowDataPacket, nhtsaCovered: boolean): Record<string, unknown> {
  return {
    brand_id: p.brand_id,
    model_id: p.model_id,
    make: p.make,
...
/**
 * `GET /recalls/internal-patterns` — fleet-internal failure-pattern intelligence.
 ...
 * FC158 T1 — renombrado desde recallsVim.ts (2026-08-13): esta es la
 * inteligencia interna de patrones de falla de la propia flota, sin relación
 * con el concepto de universo/cosmología ("VIM" era un nombre heredado de la
...
export default async function recallsInternalRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply) => {
      ...
      const data = patterns.map((p) => mapPatternRow(p, nhtsaCovered));
