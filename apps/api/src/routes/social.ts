...
// ── Posts ────────────────────────────────────────────────────────────────────
// Cada handler se extrae a función de módulo (FC166 Track D — Gate 2
// `max-lines-per-function`) para que `socialRoutes` sea solo el registro de
// rutas; mismo comportamiento verbatim en cada uno, solo el sitio cambió.
...
async function handleCreateReaction(
  request: FastifyRequest<{ Params: { id: string }; Body: ReactionBody }>,
  reply: FastifyReply
): Promise<FastifyReply> {
  try {
    await request.jwtVerify();
  ...
  if (!type || !VALID_REACTIONS.has(type as ReactionType)) {
    return reply.code(400).send({ error: 'INVALID_REACTION_TYPE' });
  }

  try {
    const [posts] = await db.execute<RowDataPacket[]>('SELECT id FROM social_posts WHERE id = ?', [
      postId,
    ]);
...
async function handleDeleteReaction(
  request: FastifyRequest<{ Params: { id: string; type: string } }>,
  reply: FastifyReply
): Promise<FastifyReply> {
  try {
    await request.jwtVerify();
  ...
  if (!VALID_REACTIONS.has(type as ReactionType)) {
    return reply.code(400).send({ error: 'INVALID_REACTION_TYPE' });
  }
...
/** Resuelve si una reseña cuenta con vínculo verificado (orden de trabajo
 * cerrada, o link owner-service) — extraída de `handleCreateReview` por el
 * mismo motivo (Gate 2); mismo comportamiento verbatim. */
async function resolveReviewVerified(
...
async function handleCreateReview(
  ...
  try {
    const verified = await resolveReviewVerified(workOrderId, linkId);
    if (!verified) {
...
/** Construye WHERE + AVG(rating) de `handleListReviews` — extraída del mismo
 * motivo (Gate 2); mismo comportamiento verbatim. */
async function fetchReviewsWithAvg(
  tallerId: string | undefined
): Promise<{ rows: RowDataPacket[]; avgRating: number | null }> {
  const conditions: string[] = [];
  const params: (string | number)[] = [];
...
async function handleListReviews(
  ...
  try {
    const { rows, avgRating } = await fetchReviewsWithAvg(tallerId);
...
/** Arma los 3 filtros (WHERE/HAVING/params) del directorio — extraída de
 * `handleListDirectory` por el mismo motivo (Gate 2); mismo comportamiento
 * verbatim. */
function buildDirectoryFilters(query: DirectoryQuery): {
  whereClause: string;
  havingClause: string;
  params: (string | number)[];
  havingParams: number[];
} {
  const { q, specialties, minRating } = query;
  ...
  return {
    ...
    havingClause: havingParts.length > 0 ? `HAVING ${havingParts.join(' AND ')}` : '',
    params,
    havingParams,
  };
}

/** Ejecuta el SELECT del directorio de talleres — extraída de
 * `handleListDirectory` por el mismo motivo (Gate 2); mismo comportamiento
 * verbatim, incluyendo el comentario forense FC 082 F3c. */
...
async function handleListDirectory(
  request: FastifyRequest<{ Querystring: DirectoryQuery }>,
  reply: FastifyReply
): Promise<FastifyReply> {
  try {
    await request.jwtVerify();
  ...
  try {
    const { whereClause, havingClause, params, havingParams } = buildDirectoryFilters(
      request.query
    );
    const rows = await fetchDirectoryRows(whereClause, havingClause, params, havingParams);
...
export default async function socialRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.get<{ Params: { id: string } }>('/social/posts/:id/comments', handleListComments);
  fastify.post<{ Params: { id: string }; Body: CommentBody }>(
    '/social/posts/:id/comments',
    handleCreateComment
  );

  fastify.post<{ Body: ReviewBody }>('/social/reviews', handleCreateReview);
