 ...
 * FC177 F3 (Cosmology_Universe_User_Linking_Backend) — `linkedUserId` on POST /universes and
 * GET /pending-users, Cond.R-177 R3 (Bravo): same TX as the tenant, fail-closed if the target
 * isn't a valid candidate (not found / already active / already a member / no billing profile),
 * MU never Ω. Supersedes FC176 F2's `initialAdmin` (inline user creation), retired by the same
 * condition — Cosmología no longer captures anyone's data by hand.
 */
...
describe('FC160 F1: /v1/cosmology/universes/:tenantId', () => {
  ...
  // ─── FC177 F3 — Cosmology_Universe_User_Linking_Backend (linkedUserId) ───
  ...
  it('COSMOLOGY-LINK-1 (Scenario 3): Ω creates a Universe WITH linkedUserId — 201, single TX migrates fiscal data + activates + membership + MU role + designation', async () => {
    (db.execute as Mock)
      .mockResolvedValueOnce([[{ id: 1, code: 'FMS', name: 'Fleet Management System' }]]) // findUniverseTypeByCode
      .mockResolvedValueOnce([[{ id: 1, code: 'FLOTILLA', name: 'Propietario de Flotilla' }]]) // findOwnerTypeByCode
    ...
    mockConnection.execute
      ...
      .mockResolvedValueOnce([{ affectedRows: 5 }]) // seedSuperclusterBlueprint
      .mockResolvedValueOnce([{ affectedRows: 1 }]) // seedClusterBlueprint
      .mockResolvedValueOnce([{ affectedRows: 1 }]) // insertTenantProfileFromBilling
      .mockResolvedValueOnce([{ affectedRows: 1 }]) // activateUser
  ...
  it("COSMOLOGY-LINK-FAILCLOSED-NOTFOUND: linkedUserId doesn't exist — 404 LINKED_USER_NOT_FOUND", async () => {
    (db.execute as Mock)
      ...
      .mockResolvedValueOnce([[{ id: 9 }]]) // findMuCosmonautRoleId
      .mockResolvedValueOnce([[]]); // findUserActiveState → not found
    const res = await app.inject({
      method: 'POST',
      url: '/v1/cosmology/universes',
      headers: omegaHeader(),
  ...
  it('COSMOLOGY-LINK-FAILCLOSED-MEMBER: linkedUserId already belongs to a Universo — 409 LINKED_USER_ALREADY_MEMBER', async () => {
    (db.execute as Mock)
      .mockResolvedValueOnce([[{ id: 1, code: 'FMS', name: 'Fleet Management System' }]])
      .mockResolvedValueOnce([[{ id: 1, code: 'FLOTILLA', name: 'Propietario de Flotilla' }]])
      .mockResolvedValueOnce([[{ id: 9 }]])
    ...
    expect(res.statusCode).toBe(409);
    expect(JSON.parse(res.body).code).toBe('LINKED_USER_ALREADY_MEMBER');
    expect(mockConnection.beginTransaction).not.toHaveBeenCalled();
  });

  it('COSMOLOGY-LINK-FAILCLOSED-NOBILLING: linkedUserId never completed public signup fiscal data — 409 LINKED_USER_MISSING_BILLING_PROFILE', async () => {
    (db.execute as Mock)
      .mockResolvedValueOnce([[{ id: 1, code: 'FMS', name: 'Fleet Management System' }]])
      .mockResolvedValueOnce([[{ id: 1, code: 'FLOTILLA', name: 'Propietario de Flotilla' }]])
      .mockResolvedValueOnce([[{ id: 9 }]])
    ...
    expect(res.statusCode).toBe(409);
    expect(JSON.parse(res.body).code).toBe('LINKED_USER_MISSING_BILLING_PROFILE');
    expect(mockConnection.beginTransaction).not.toHaveBeenCalled();
  });

  it('COSMOLOGY-LINK-ROLLBACK (Cond.R-177 R3 "MISMA TX que tenant"): linking failure mid-TX rolls back the tenant too', async () => {
    (db.execute as Mock)
      .mockResolvedValueOnce([[{ id: 1, code: 'FMS', name: 'Fleet Management System' }]])
      .mockResolvedValueOnce([[{ id: 1, code: 'FLOTILLA', name: 'Propietario de Flotilla' }]])
      .mockResolvedValueOnce([[{ id: 9 }]])
    ...
    mockConnection.execute
      ...
      .mockResolvedValueOnce([{ affectedRows: 5 }]) // seedSuperclusterBlueprint
      .mockResolvedValueOnce([{ affectedRows: 1 }]) // seedClusterBlueprint
      .mockRejectedValueOnce(new Error('DB connection lost mid-link')); // insertTenantProfileFromBilling
    const res = await app.inject({
      method: 'POST',
      url: '/v1/cosmology/universes',
      headers: omegaHeader(),
  ...
  it('COSMOLOGY-LINK-BOLA (Anti-BOLA): non-Ω actor with linkedUserId payload — 403, 0 queries', async () => {
    const res = await app.inject({
      method: 'POST',
      url: '/v1/cosmology/universes',
      headers: arcHeader(),
  ...
  it('COSMOLOGY-PENDING-NOAUTH: GET /pending-users — 401 without a JWT', async () => {
    const res = await app.inject({ method: 'GET', url: '/v1/cosmology/pending-users' });
    expect(res.statusCode).toBe(401);
  });
