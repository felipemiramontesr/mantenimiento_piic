 ...
 * Returns { status: "ok", ts: epoch_ms }. (FC163 F2B3, split Alfa 227_AN.)
 */
 ...
 */
export async function handleFamilyUnits(
  request: FastifyRequest,
  reply: FastifyReply
): Promise<FastifyReply> {
  try {
...
export async function handleTelemetryPing(
  request: FastifyRequest<{ Body: TelemetryPingBody }>,
  reply: FastifyReply
): Promise<FastifyReply> {
  try {
    await request.jwtVerify();
 ...
 * (FC163 F2B3, split Alfa 227_AN.)
 */
export async function handleTelemetryUnits(
  request: FastifyRequest,
  reply: FastifyReply
): Promise<FastifyReply> {
  try {
  ...
  return reply.send({
    units: rows.map((r) => ({
      ...
      speed: Number.parseFloat(r.speed),
      heading: Number.parseFloat(r.heading),
