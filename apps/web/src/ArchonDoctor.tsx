  ...
  return (
    <div className="fixed bottom-4 right-4 z-[9999] w-[450px] h-[600px] bg-pinnacle-navy border border-pinnacle-yellow/30 shadow-2xl rounded-lg flex flex-col overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
      ...
      <div className="p-4 bg-pinnacle-navy border-b border-pinnacle-yellow/10 flex items-center justify-between">
        ...
        <button onClick={() => setIsOpen(false)} className="text-pinnacle-white/40 hover:text-pinnacle-white">
          ✕
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 font-mono text-[11px] custom-scrollbar">
        {activeTab === 'NET' && (
          <div className="space-y-4">
            <div className="p-3 bg-black/30 rounded border border-white/5">
              <p className="text-pinnacle-yellow/60 uppercase text-[9px] mb-2 tracking-tighter">Gateway Status</p>
              <div className="grid grid-cols-2 gap-2 text-pinnacle-white/80">
                ...
                <span>JWT_AUTH:</span> <span className={context?.isSyncing ? 'text-green-400' : 'text-yellow-400'}>DETECTOR ACTIVE</span>
              </div>
            </div>
            <div className="space-y-1">
              {logs.filter(l => l.type !== 'data').length === 0 ? (
                ...
                logs.filter(l => l.type !== 'data').map((log, i) => (
                  <div key={i} className={`flex gap-2 ${log.type === 'err' ? 'text-red-400' : 'text-pinnacle-white/60'}`}>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="p-3 bg-black/30 rounded border border-red-500/20">
                ...
                <p className="text-2xl font-black text-pinnacle-white">{context?.integrity?.corrupt || 0}</p>
              </div>
            </div>
            
            <div className="p-3 bg-black/30 rounded border border-white/5">
              <p className="text-pinnacle-yellow/60 uppercase text-[9px] mb-2">Structure Integrity</p>
              <div className="space-y-1 text-pinnacle-white/60">
                ...
                <div className="flex justify-between border-b border-white/5 pb-1">
                  <span>Stats Total:</span> <span className="text-pinnacle-white">{context?.stats?.total || 0}</span>
                </div>
              </div>
            </div>
        ...
        {activeTab === 'ERR' && (
          <div className="space-y-2">
            {logs.filter(l => l.type === 'err').length === 0 ? (
              <p className="text-green-400/40 text-center py-10 tracking-widest">ZERO CRITICAL EXCEPTIONS DETECTED</p>
            ) : (
              logs.filter(l => l.type === 'err').map((log, i) => (
        ...
        {activeTab === 'CACHE' && (
          <div className="space-y-4">
             <div className="p-3 bg-black/30 rounded border border-white/5">
                <p className="text-pinnacle-yellow/60 uppercase text-[9px] mb-2 tracking-tighter">Persistence Layer</p>
                <p className="text-pinnacle-white/80">Prefix: <span className="text-blue-400">archon_</span></p>
             </div>
             <button 
              onClick={() => {
